...
function computeLevel(parentNode, parentMask, gs, iters, smooth, out, depth, onProg, total, doneRef, containerPolys) {
  ...
  // For non-convex containers the analytical cells are clipped to the convex hull,
  // so raw polyArea over-counts area in the concave notches (e.g. star indentations).
  // Pre-triangulate the ACTUAL container polygons with earClipTriangulate so the
  // refinement loop can clip each cell against those triangles and measure only the
  // area that truly lies inside the container.  For convex containers the fast path
  // (direct polyArea on the already-correctly-clipped cell) is used instead.
  var containerTris=null; // array of [A,B,C] triangles covering the real container
  if(!singleConvex && containerPolys && containerPolys.length){
    containerTris=[];
    for(var _ci=0;_ci<containerPolys.length;_ci++){
      var _tris=earClipTriangulate(containerPolys[_ci]);
      for(var _ti=0;_ti<_tris.length;_ti++) containerTris.push(_tris[_ti]);
    }
    if(!containerTris.length) containerTris=null; // degenerate — fall back to fast path
  }

  // Compute the area of an array of boundary polygons clipped to the real container.
  // Uses triangle decomposition for non-convex containers; direct polyArea otherwise.
  function cellAreaInContainer(bndPolys){
    if(!containerTris){
      var a=0; for(var _p=0;_p<bndPolys.length;_p++) a+=polyArea(bndPolys[_p]); return a;
    }
    var a=0;
    for(var _p=0;_p<bndPolys.length;_p++){
      for(var _t=0;_t<containerTris.length;_t++){
        var _tri=containerTris[_t];
        var _clipped=clipToTriangle(bndPolys[_p],_tri[0],_tri[1],_tri[2]);
        if(_clipped.length>=3) a+=polyArea(_clipped);
      }
    }
    return a;
  }
  ...
  for(var ref=0;ref<REFINE_ITERS;ref++){
    ...
    for(var i=0;i<n;i++){
      aAreas[i]=cellAreaInContainer(analyticalBnds[i]);
      aTotal+=aAreas[i];
    }
    if(aTotal<1) break;
    var changed=false;
  ...
  // For non-convex containers: clip each analytical cell against the actual container
  // triangles so child levels receive the true within-container shape, not the inflated
  // convex-hull cell.  This fixes child area proportions (e.g. Canada vs Mexico).
  var starClippedBnds=null;
  if(containerTris){
    starClippedBnds=new Array(n);
    for(var i=0;i<n;i++){
      var pieces=[];
      var srcBnd=analyticalBnds[i];
      for(var _p=0;_p<srcBnd.length;_p++){
        for(var _t=0;_t<containerTris.length;_t++){
          var _tri=containerTris[_t];
          var _clipped=clipToTriangle(srcBnd[_p],_tri[0],_tri[1],_tri[2]);
          if(_clipped.length>=3) pieces.push(_clipped);
        }
      }
      starClippedBnds[i]=pieces.length?pieces:srcBnd;
    }
  }
  ...
  for(var i=0;i<n;i++){
    ...
    if(analyticalBnds[i]&&analyticalBnds[i].length){
      ...
      // Pass star-clipped boundary as child container so child refinement uses
      // the true within-container shape for area feedback, not the convex-hull cell.
      cellContainerPolys=starClippedBnds&&starClippedBnds[i]&&starClippedBnds[i].length
        ? starClippedBnds[i] : analyticalBnds[i];
    } else {
